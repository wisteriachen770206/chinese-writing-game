        ...
        // This placeholder is replaced at build time (itch.io zip) with the current git commit hash.
        // Example after build: "1a2b3c4d5e6f..."
        const BUILD_GIT_COMMIT = '925017880858f542deb54cac6b75c3c50a112656';

        function getBuildGitCommitOrNull() {
            const v = String(BUILD_GIT_COMMIT || '').trim();
            // Accept 7-40 hex chars (short or full SHA)
        ...
        async function loadLevelConfig() {
            try {
                const v = Date.now();
                const commit = getBuildGitCommitOrNull();
                const cdnBase = commit
                    ? `https://cdn.jsdelivr.net/gh/wisteriachen770206/chinese-writing-game@${commit}`
                    : `https://cdn.jsdelivr.net/gh/wisteriachen770206/chinese-writing-game`;

                // If you're running locally, try local first; on itch/CDN environments, try CDN first.
                const isLocalDev =
                    (location && (location.hostname === 'localhost' || location.hostname === '127.0.0.1')) ||
                    (location && location.protocol === 'file:');

                const cdnUrl = `${cdnBase}/level_config.json?v=${v}`;
                const localUrl = `level_config.json?v=${v}`;
