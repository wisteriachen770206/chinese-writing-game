...
async function getStrokeData(page, character) {
    return await page.evaluate(async (char) => {
        return new Promise((resolve, reject) => {
            // Load hanzi-writer from CDN
            const script = document.createElement('script');
            script.src = 'https://cdn.jsdelivr.net/npm/hanzi-writer@3.2.0/dist/hanzi-writer.min.js';
            script.onload = () => {
                try {
            ...
            script.onerror = () => reject(new Error('Failed to load hanzi-writer'));
            document.head.appendChild(script);
            
            // Create a temporary div for the writer
...
async function processAllCharacters() {
    ...
    await page.setContent(`
        <!DOCTYPE html>
        <html>
        <head><title>Stroke Data Extractor</title></head>
        <body><div id="temp"></div></body>
        </html>
    `);
    ...
    for (let i = 0; i < characters.length; i++) {
        ...
        try {
            const charData = await getStrokeData(page, char);
